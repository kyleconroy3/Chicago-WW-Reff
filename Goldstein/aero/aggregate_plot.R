...
run <- function(cfg) {
    ...
    stickney_south <- readRDS(cfg$stickney_south)# readRDS("/lcrc/project/EMEWS/bebop-2.0/ncollier/repos/Chicago-WW-Reff/Goldstein/aero/output/stickley_south_rt_quantiles_20250623_130859.csv")
    calumet <-  readRDS(cfg$calumet)# readRDS("/lcrc/project/EMEWS/bebop-2.0/ncollier/repos/Chicago-WW-Reff/Goldstein/aero/output/calumet_rt_quantiles_20250623_130859.csv")
    stickney_north <- readRDS(cfg$stickney_north)
    obrien <- readRDS(cfg$obrien)

    overalloverlap_combined <- bind_rows(
        stickney_south %>% mutate(plot = "Stickney South"),
        stickney_north %>% mutate(plot = "Stickney North"),
        calumet  %>% mutate(plot = "Calumet"),
        obrien %>% mutate(plot = "O'Brien")
    )
    ...
    pop_stickney_north <- 1127736
    pop_stickney_south <- 1127737
    pop_obrien <- 1263110
    pop_calumet <- 1134897
    
    Sewershed_combined2$population[Sewershed_combined2$plot == "Stickney South"] <- pop_stickney_south
    Sewershed_combined2$population[Sewershed_combined2$plot == "Stickney North"] <- pop_stickney_north
    Sewershed_combined2$population[Sewershed_combined2$plot == "Calumet"] <- pop_calumet
    Sewershed_combined2$population[Sewershed_combined2$plot == "O'Brien"] <- pop_obrien
    ...
    # geom_hline(yintercept = 1, linetype = "solid", color = "black", lwd=1)
    # aggregate_CI4
    # ggsave("./output/aggregate_CI4_plot.png", plot = aggregate_CI4, width = 15, height = 6)

    head(agg_estimates4)

    combined_plot <- ggplot(agg_estimates4, aes(x = numdate, y = agg_mean)) +
        geom_line(size = 1.5, col = "red") +
        geom_line(data = stickney_south, aes(x = date, y = value), col = "blue", size = 0.8) +
        geom_line(data = calumet, aes(x = date, y = value), col = "black", size = 0.8) +
        geom_ribbon(aes(ymax = ci_upper, ymin = ci_lower), 
    ...
    combined_plot
    plot_fname = paste0(cfg$out_dir, "/", cfg$aggregate_plot_name)
    ggsave(plot_fname, plot = combined_plot, width = 15, height = 6)
