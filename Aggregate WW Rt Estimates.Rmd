...
combined_plot <- ggplot(agg_estimates4, aes(x = numdate, y = agg_mean)) +
  geom_line(size = 1.5, col = "red") +
  geom_line(data = odata, aes(x = date, y = value), col = "blue", size = 0.8) + #Adds line for each individual WW trajectory - data = individual processed data that goes into 'overalloverlap_combined' on line 29
  geom_line(data = cdata, aes(x = date, y = value), col = "black", size = 0.8) +
  geom_ribbon(aes(ymax = ci_upper, ymin = ci_lower), 
